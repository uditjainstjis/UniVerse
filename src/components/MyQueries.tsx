...
const MyQueries: React.FC<MyQueriesProps> = ({ userQueries, onReply, onResolve }) => {
  return (
    <div className="max-w-4xl mx-auto pt-8 pb-24 px-4">
      <div className="mb-12">
        <h2 className="text-4xl font-black text-white tracking-tighter mb-2">My Queries</h2>
        <p className="text-white/40 text-sm font-medium">Track and manage your campus questions.</p>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {userQueries.length === 0 ? (
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="p-12 rounded-[40px] border border-dashed border-white/10 text-center space-y-4 opacity-40"
            >
